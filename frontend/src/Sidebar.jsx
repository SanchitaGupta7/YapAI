...
function Sidebar() {
  ...
  const deleteThread= async (e, threadId)=>{
    ...
    try{
      if(!threadId) return;
      await fetch(`https://yapaibackend.onrender.com/api/thread/${threadId}`, {
        method:"DELETE"
      });
  ...
  const changeThread= async (threadId)=>{
    ...
    try{
      const response= await fetch(`https://yapaibackend.onrender.com/api/thread/${threadId}`);
  ...
  const getAllThreads = async () => {
    try{
      const response = await fetch("https://yapaibackend.onrender.com/api/thread");
      const res = await response.json();
